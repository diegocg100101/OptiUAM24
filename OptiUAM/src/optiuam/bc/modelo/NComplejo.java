...
/**
 * Clase NComplejo la cual contiene los atributos principales y metodos de un numero complejo
 * @author Arturo Borja
 * @author Karen Cruz
 */
public class NComplejo{
    ...
    public NComplejo(double real, double imag){
        this.real=real;
        this.imag=imag;
    }
    
    public static NComplejo conjugado(NComplejo c){
       return new NComplejo(c.real, -c.imag);
    }
    
    public static NComplejo opuesto(NComplejo c){
       return new NComplejo(-c.real, -c.imag);
    }
    ...
    /**Magnitud de un numero complejo
     * @return Magnitud*/
    ...
    public static NComplejo suma(NComplejo c1, NComplejo c2){
       double x=c1.real+c2.real;
       double y=c1.imag+c2.imag;
       return new NComplejo(x, y);
    }
    
    //producto de dos números complejos
    public static NComplejo producto(NComplejo c1, NComplejo c2){
    ...
    public static NComplejo producto(NComplejo c, double d){
       double x=c.real*d;
       double y=c.imag*d;
       return new NComplejo(x, y);
    }
    ...
    public static NComplejo producto2(double d, NComplejo c){
       double x=c.real*d;
       double y=c.imag*d;
       return new NComplejo(x, y);
    }
    ...
    public static NComplejo cociente(NComplejo c1, NComplejo c2)throws ExcepcionDivideCero{
       double aux, x, y;
       if(c2.modulo()==0.0){
    ...
    public static NComplejo cociente(NComplejo c, double d)throws ExcepcionDivideCero{
      double x, y;
      if(d==0.0){
            throw new ExcepcionDivideCero("Divide entre cero");
      }else{
    ...
    public static NComplejo exponencial(NComplejo c){
    ...
    public static NComplejo csqrt(double d){
       if(d>=0) return new NComplejo(Math.sqrt(d), 0);
       return new NComplejo(0, Math.sqrt(-d));
    }
    ...
    public static NComplejo potencia(NComplejo c, int exponente){
      double x=0.0, y=0.0;
      int signo;
      for(int i=0; i<=exponente; i++){
          signo=(i%2==0)?+1:-1;
